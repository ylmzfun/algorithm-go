...
func TestGetLongestCommonPrefix(t *testing.T) {
	...
	// 测试有公共前缀的情况
	// application/apply/apple 共享 a-p-p-l，到第 5 个字符（i/y/e）才分歧，
	// 因此最长公共前缀是 "appl"
...
func TestGetShortestUniquePrefix(t *testing.T) {
	...
	// 测试 "apple" 的最短唯一前缀
	// apple 与 application 共享到 "appl"，第 5 个字符（e/i）才分歧，
	// 因此 "apple" 的最短唯一前缀是其本身 "apple"
	...
	// 测试 "banana" 的最短唯一前缀
	// banana 与 band 共享到 "ban"，第 4 个字符（a/d）分歧，因此是 "bana"
